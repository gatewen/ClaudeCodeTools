...
| 版本 | 重點 |
|------|------|
| **v7.1.2** | **交接工具 `/dev:handoff` 健壯性強化 + 誠實校正**（patch · 2026-06-15 · 需重裝）——一份外部評估報告挑出幾個真問題，用多代理 workflow + 對抗驗證收斂後修掉：(1) **路徑 bug**：含底線的專案名（如 `UK_Wrok`）在某種模式下，交接檔會被寫到一個跟系統實際資料夾對不上的「孤兒目錄」（系統會把底線也換成短橫，舊程式沒有）——已修，並把一句不實的「跟系統編碼一致」說明改回實話。(2) **誠實校正**：先前自述跟個人版 `wt:handoff`「逐字相同」，實測已不符（5 個檔有 1 個分化了），改成精確描述。(3) **防呆**：載入交接、重建待辦前，先用 git 查一下「這件事是真沒做、還是做了只是沒提交」，避免照交接字面把已完成的工作覆蓋掉；偵測到可疑的外部交接檔會先備份再覆蓋。(4) 新增一張「`/compact` 跟 `/dev:handoff` 什麼時候各用哪個」的對照表。誠實說明：這版是**修漏洞+講實話+補一條小紀律**，不是加新功能。重裝：`git pull && bash setup.sh` 或重跑 curl 安裝指令 |
| **v7.1.1** | **修好 v7.1.0 在 Windows 上 `/dev:handoff`、`/dev:overview` 指令沒出現的 bug**（patch · 2026-06-03 · 需重裝）——v7.1.0 發佈後在 Windows 實測：打 `/dev:` 只出現 `/dev:init-claude`，那兩個指令都不見。用多代理對抗驗證查出兩個原因：(1) `overview` 指令檔開頭的設定區有個 YAML 格式錯誤（一段說明文字裡的「冒號+空格」被誤判成結構）；(2) 兩個指令檔都多寫了一個「帶冒號的名稱欄位」，跟「靠資料夾自動命名」衝突。都修好了（拿掉多餘名稱欄位、格式改成安全的折疊塊寫法），並加了自動測試擋這類錯回來。**仍待你在 Windows 確認**：還有一個可能性是 git 在 Windows 拉這版時因為舊的冒號資料夾中斷、導致新檔根本沒下載——若重裝後還是沒出現，請把 `git pull` 的訊息回報。重裝：`git pull && bash setup.sh` 或重跑 curl 安裝指令 |
