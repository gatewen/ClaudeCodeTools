...
| 版本 | 重點 |
|------|------|
| **v7.0.0** | **workflow-first 重構**（breaking · 2026-06-01）——A–F dogfood 實證五階流水線 ritual 對 correctness 零增益（六場三方平手），故把編排交給 Claude Code 原生 Workflow：新增 4 個 workflow 腳本（`/dev-prd` `/dev-design` `/dev-review` `/dev-verify`），CLAUDE.md 縮減約 42%（588→342 行）。三層架構：**L1 always-on hook**（因果鏈 fail-safe 地基）+ **L2 workflow**（預設首選編排，內嵌承重核 + 對抗驗證）+ **L3 文字層**（workflow 不可用時退化路徑，Section 14）。承重核（因果鏈+事實求證）重新定位：correctness 軸已證零增益，價值錨在人軸（把握度校準/發現成本攤銷/前提層防禦·人軸 proxy 未否證≠已證）。8-agent 委派降為 workflow prompt 素材庫；ID 系統簡化（主軸 BC-x+R-x）。兩輪 R-2 cross-source review（8 high 全解）。⚠️ workflow 需 v2.1.154+ · 付費 · research preview |
| **v6.5.0** | 公開發佈友善度大躍進：新增 `dev:overview` Skill — 產生方法論視覺化介紹 HTML 給人類看。新手 30 秒理解、進階用戶按需展開 11 個 section 細節，含 5 phase 流程圖 + 認知驗證三層防禦 SVG + 升格降級狀態機 + Hook/Agent/對照範例/語言指南/工具鏈介紹。Self-contained HTML（離線可開）+ 右上角 light/dark mode 切換（localStorage 持久化，不依系統設定）。已部署 cwd 自動填當前部署狀態（版本 / 啟用功能 / 累積活動）。隨 setup.sh 一鍵部署到 `~/.claude/skills/dev:overview/`，跑 `/dev:overview` 即生成 `.claude-loop/overview.html`（已部署）或 `cwd/dev-overview.html`（未部署） |
| **v6.4.2** | source-of-truth marker 三次漏改修救（process patch）：發現 v6.4.0 / v6.4.0 follow-up / v6.4.1 三次 release 都漏改 CLAUDE_TEMPLATE.md 末尾 `closed-loop v` source-of-truth marker，導致 GitHub remote / cache / 部署版的 source-of-truth 全停在 v6.3.0，跑 `/dev:init-claude upgrade` 觸發 STATUS=up_to_date 靜默失敗。修救：marker 跳轉 v6.3.0 → v6.4.2（內容涵蓋 v6.4.0 升格降級機制 + v6.4.1 dev:handoff fork 全部變更）。Process patch：依賴影響表「版本號」行明指要改的是哪一行 marker（不只是「末尾註解」籠統描述）+ learning-log 加觀察條目（不升格，先觀察是否擴散） |
