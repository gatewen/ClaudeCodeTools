...
每次 Edit/Write/MultiEdit 前，Hook 自動掃描被修改檔案的引用者，輸出依賴清單和因果鏈提醒。此提醒等同用戶回饋，**必須在畫面上輸出分析結果後才執行修改**——不能靜默處理。

#### 可見性規則（必須在畫面輸出）

收到 Hook 提醒後，**必須輸出因果鏈分析區塊**讓用戶看到，格式：
...
### 12. 理解確認（UserPromptSubmit Hook）

用戶提交指令時，Hook 自動觸發橙色提醒。收到提醒後，**在執行任何修改前**，必須先在畫面輸出理解確認：
