...
**語言指南**：若已部署語言指南，先讀取 Phase 3 段落（審查清單、安全/效能反模式）。
...
**步驟 1 — 設計**：簡要設計規格（目標 | 主要函式簽名與參數型別 | BC-x ≥ 1 | EH-x 覆蓋主要異常）。若已部署語言指南，參考語言慣例設計。
**步驟 2 — 實作**：按設計實作。每完成一個檔案立即執行 `{{LINT_COMMAND}}` 驗證，發現錯誤當場修正。單檔超過 300 行須拆分。全部完成後調用 Task `code-simplifier` 優化。
**步驟 3 — 驗證**：
