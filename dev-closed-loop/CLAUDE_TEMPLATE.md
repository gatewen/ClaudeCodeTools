...
- 若模組依賴其他模組的公開 API → 從 `.claude-loop/interfaces/` 讀取 IF-x，設計規格引用而非自行描述
...
```
步驟 1：建立基準清單
  從 Phase 1 的設計規格中，列出所有 BC-x、EH-x 和 IF-x（若有）的 ID
  這就是「基準清單」，後面所有比對都以此為準

步驟 2：設計-實作一致性
  對照基準清單，逐一確認 Phase 2 的程式碼中有沒有對應實作
  每個 ID 標注：✅ 有實作 / ❌ 缺漏（記錄具體缺了什麼）
  IF-x 需額外確認：實作的函式簽名和資料格式是否與介面契約一致
...
| IF-1, IF-2... | 介面契約（Interface contract） | 持久化啟動時定義 → Phase 1, 2, 5 引用 |
| CR-1, CR-2... | 變更請求（Change Request） | 介面變更時建立 → 受影響模組引用 |
...
IF-x 是跨模組公開 API 的唯一真相來源。提供模組和消費模組都引用同一個 IF-x，不各自描述介面。
...
「影響分析」填完後，同步更新 `project-state.md` 和受影響模組的 `status.md`。
...
| 介面契約一致性 | ✅/❌ | [例：IF-1, IF-2 的實作均符合契約定義] | [若有不一致寫這裡] |

> 「介面契約一致性」僅在模組有 IF-x 時出現。方法層級閉環不需要此維度。
...
| Phase 1 定義跨模組 API | `interfaces/IF-{n}.md` |
| 模組修改了 IF-x 介面 | `changes/CR-{n}.md` + 更新受影響模組的 `status.md` |
...
| 進入依賴其他模組的閉環 | 相關 `interfaces/IF-{n}.md` |
| 模組閉環恢復 | `changes/` 中狀態為「待處理」的 CR-x |

### 專案初始化（首次啟用持久化時）

首次啟用持久化時，按以下順序建立初始結構：

1. 定義模組清單和依賴關係 → 寫入 `project-state.md`
2. 為每個跨模組依賴定義介面契約 → 寫入 `interfaces/IF-{n}.md`（格式見「產出物格式」章節）
3. 按依賴圖的拓撲順序，從最底層模組開始閉環

若介面尚不明確（需求還在探索中），可以先建立「草案」狀態的 IF-x，在模組閉環過程中逐步確認。

詳細的目錄格式、檔案模板和操作範例：[跨 Session 持久化](.claudedocs/process/跨Session持久化.md)
介面契約和變更請求的詳細規則和範例：[介面契約與變更管理](.claudedocs/process/介面契約與變更管理.md)
...
| 9   | [介面契約與變更管理](.claudedocs/process/介面契約與變更管理.md) | 跨模組介面的 IF-x 契約和 CR-x 變更追蹤 |
