...
- **單一前沿模型能在單一 context 可靠處理的任務**（規格清楚、小到中大型——dogfood 實證已涵蓋 review / 建小 / 建大牽連(~900 行 4 模組) / 改碼漣漪 / 假前提五型）：此 regime 下閉環對**可量測的結果零增益**、成本最高約裸寫的 7x（見下方校準）

> ⚖️ **價值定位校準（2026-05-30 dogfood A–E 對照實驗）**：五型任務（A review 找植入 bug / B 從規格建小 / C 從規格建大牽連 / D 改既有碼隱藏漣漪 / E 植入假前提）× 三臂（裸寫一次 / 裸寫+一次外部 LLM review / 完整閉環）+ Codex 凍結的 held-out 測試，**每一場都三方平手**，涵蓋代碼正確性 / 質量維護 / 修改因果鏈 / 事實求證四維度——閉環對「單一模型一次性產出的可量測結果」零增益，成本 +40%～+590%（最高約裸寫 7x）。
> **核心洞察**：閉環把「前沿模型認真做事時本來就會做的認知」（讀全碼 / 影響分析 / 把事實宣稱拿去核對證據 / 枚舉邊界）外部化、儀式化了。模型已在做 → 對它自己的代碼正確性是淨成本；這份外部化的價值在**人**（可追溯 / 可稽核 / 跨 session 防失憶 / 團隊協作），correctness oracle 測不到。
> ∴ **把閉環用在它有優勢的地方（大型 / 長時程多 session / 多人 / 需稽核），別用在它淨負的地方（上列不適用情境）。** 校準限制：每型 n=1、前沿模型、單一 context 任務；超出單一 context 可靠度的超大型 / 長時程 regime 是唯一尚未否證、閉環仍可能有價值之處。
>
> ⚖️ **Stage F 追加校準（2026-05-30，打上面那個「未否證 regime」的一角）**：用 Evict-then-Edit 實測「B（被依賴的遠端不變式）不在當前 context 窗內」——跨 session 預建 codebase + 新增列舉案例 + 遠端消費點漏接會靜默失效。結果：前沿模型裸寫一個 `grep` 就把窗外消費點全找回來補上（無歧義的靜默 bug 三臂全平手）；唯一分歧是一個「需求未指定的設計選擇」污染點，且**一份自由 note = 一次外部 review = 完整五階段（最貴）全部同分，ritual 零邊際貢獻**。
> ∴ **correctness 軸（A–F 六場）已耗盡為 robust null，連「B 不在窗」也是；勿再用 agent-to-agent correctness 測試（含放大尺度——前沿模型仍會 grep 平手）找價值。** 唯一真正未測、且本方法論理論上才有獨佔優勢的，是**「人」軸**：人類接手/稽核成本、可向 stakeholder「證明」覆蓋（非僅達成）、以及弱執行者（人類 / 長自主 agent / 弱模型，非前沿模型 fresh context）的紀律護欄——這些需用**非 correctness-oracle 的指標**衡量。
