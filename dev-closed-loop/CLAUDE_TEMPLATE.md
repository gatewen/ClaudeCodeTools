...
### 9. 因果鏈分析守衛（PreToolUse Hook — 阻擋式）

每次 Edit/Write/MultiEdit 前，Hook 自動掃描被修改檔案的引用者。**首次修改某檔案時，Hook 會阻擋操作（exit 2）**，強制你先輸出因果鏈分析；分析完成後重試修改，Hook 偵測到已分析過即放行。

這不是「提醒」——是真正的門鎖。沒做分析，修改就不會執行。

#### 運作機制

```
第一次 Edit foo.ts → Hook 掃描引用者 → ⛔ 阻擋 + 輸出依賴清單
                     ↓
你輸出因果鏈分析給用戶看
                     ↓
第二次 Edit foo.ts → Hook 偵測已分析 → ✅ 放行
```
...
### 12. 理解確認（UserPromptSubmit Hook — 提醒式）

用戶提交指令時，Hook 自動觸發橙色提醒（注意：此 Hook 為提醒機制，不像 Section 9 的因果鏈守衛會阻擋操作）。收到提醒後，**在執行任何修改前**，必須先在畫面輸出理解確認：
