...
| **大型** | 新模組/功能 · ≥ 3 檔案或 ≥ 300 行 · 用戶說「完整閉環」 | 完整五階段閉環（Phase 1→5） |
...
**約束**：只產出設計規格，不寫程式碼。BC-x ≥ 2、所有參數有型別、EH-x 覆蓋可預見異常。若涉及跨模組 API → 從 `.claude-loop/interfaces/` 讀取或產出 IF-x 介面契約。
**⛔ 閘門**：參數有型別 ∧ BC ≥ 2 ∧ EH 覆蓋異常 → 才能進 Phase 2。
