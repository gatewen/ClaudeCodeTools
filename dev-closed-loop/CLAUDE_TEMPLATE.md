...
- **TaskCreate**：觸發閉環時立即建立任務鏈（大型 6 個 / 中型 3 個），讓用戶看到進度
...
**⛔ 閘門**：參數有型別 ∧ BC ≥ 2 ∧ EH 覆蓋異常 ∧ status 變更有行為約束 ∧（若有 PRD：所有 PRD#n 有 ≥ 1 對應 BC-x/EH-x）→ 才能進 Phase 1b。
...
**⛔ 閘門**：
- 有 DR-x high → 必須回 Phase 1 修正設計，修正後重跑 Phase 1b
...
**獨立性要求**：品質審查和安全審查各使用 Task 啟動獨立子 agent，**可並行**發送。合併 R-x 結果後統一判定斷點 A。
**品質審查 Task prompt**：
...
**自証（三段式）**：
> Part A 與 Part B 由獨立子 agent 執行（不繼承主對話 context），**可並行**發送。Part C 由主 agent 彙整判定。
> 多模組任務：自証涵蓋所有模組的設計項、程式碼和測試。Part C 的全專案回歸測試為必做。

**Part A — 追溯檢查表**（獨立子 agent · 設計 → 實作 → 測試）：
...
- Phase 2 程式碼檔案路徑清單（子 agent 用 Read 自行讀取分析）

**Task prompt 必須包含**：
- 角色：「你是獨立自証審查者（反向分析），與開發者無關，不知道開發過程」
...
**回退規則**：設計-實作不一致 → P2（嚴重→P1）| 測試不足 → P4 | 檢核未修 → P2 | DR-x high 未修 → P1 | DR-x 決策未記錄 → 補記錄 | 產出物缺漏 → 對應 Phase
