...
1. **兩層教訓查詢**（必做，順序固定）：
   - **a. 長期模式優先**：必讀 `.claudedocs/records/問題追蹤.md`「長期警惕模式」section（這是跨閉環累積且 ≥ 3 次升格的高頻模式，**永遠不可跳過**）。掃描每筆條目的「觸發情境」與當前需求比對，命中的條目把「預防做法」納入本次設計考量
     - **⏸️ 條件式標記識別**（v6.4.0 新增）：掃描時若條目標 `⏸️ 條件式`（位於同檔「條件式紀律」section），先讀「觸發條件」欄位，僅當當前需求命中觸發條件時才把預防做法納入考量；否則略過該條目（避免污染無關設計）。標 `🗄️ archived` 條目一律略過
...
**步驟 1.5 — 把命令式指令轉成可驗證目標（v6.1.0 新增 · Karpathy 學習 · K-08）**

用戶指令通常是命令式（「加驗證」「修 bug」「優化效能」）。進入 BC-x 設計前，先做轉換：
...
- [ ] 驗證層級已標注
- [ ] 分層結構已聲明
- [ ] **資深工程師審視通過**（v6.1.0 新增 · K-02）：退一步看，一位資深工程師會說此設計「過度設計」嗎？
...
## 實作規模估算與 3x rule（v6.1.0 新增 · Karpathy 學習 · K-03）
...
**反模式**：估算當上限（×，估算是預期不是上限）/ 用「複雜」為理由規避（×，必須具體說明哪段業務真的需要這麼多行）。
