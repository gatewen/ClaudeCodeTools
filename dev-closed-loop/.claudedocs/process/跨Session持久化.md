...
> **v7.0.0 定位**：持久化機制在 v7 **仍有效並被沿用**——v7 大型任務 workflow 的產出同樣寫入 `.claude-loop/artifacts/`（見主檔「設計規格持久化」），後續步驟 / session 從此讀取。本文件的 Phase 標籤（Phase 1-5、`P1b-design-review.md` 等檔名）為 v6.x 委派產物命名；v7 由 workflow 編排（`/dev-design` `/dev-review` `/dev-verify`）產出同目錄檔案。IF-x / CR-x 為 v7 按需 inline 機制（見[介面契約與變更管理](介面契約與變更管理.md)）。
...
> 下表「誰寫入 / 誰讀取」欄為 **v6.x 委派模型**（主 agent 逐 Phase 委派 + design-reviewer / code-reviewer / verifier sub-agent）。v7 預設由 workflow 編排寫入同目錄檔案（不再走主 agent 逐 Phase 委派）；workflow 不可用時退化路徑才回到此委派模型。
...
最後修訂：2026-06-01（v7.0.0 對齊：頂部加 v7 定位 — 持久化機制 v7 仍沿用，workflow 產出寫入同 artifacts/ 目錄；Phase 標籤定位為 v6.x 委派產物命名；「不改變五階段」改為「不改變編排與承重核」）
