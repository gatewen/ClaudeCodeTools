...
- 分層審查標準（功能+UI 分層的模組）：功能層的公開 API 是否足夠抽象（不依賴 UI 框架的型別或元件）？依賴方向是否正確（功能層 → UI 層，不可反向）？功能層是否有複用潛力？（功能層依賴 UI 框架 → 標 high）
...
- 審查規則：比對設計與實作一致性 + 結構安全（資源生命週期配對/錯誤靜默/非法狀態組合）+ 依賴方向（功能+UI 分層的模組：功能層是否有直接 import UI 框架的元件/型別？有 → R-x high。功能層 API 回傳值是否包含 UI 框架特定型別？有 → R-x medium）+ 語言專屬審查（標 `[語言名]`）+ 跨模組資料流追蹤（拆分邊界的資料過濾/轉換是否保持下游模組預期的語意）+ 合理性審查（實作結果從使用者角度看是否合理：操作流程有無變複雜？多個功能組合使用時體驗是否順暢？有無引入不必要的效率退步？）
